...
def listar_itens_sync(nfs: int):
    con = get_connection()
    cur = con.cursor()
    cur.execute("SELECT * FROM NFS_ITENS WHERE NFS = ?", (nfs,))
...
async def listar_itens(
    ...
    nfs: int = Query(..., description="Número da nota fiscal de saída (NFS)")
):
    if not nome or not is_authorized(nome):
        raise HTTPException(status_code=403, detail="Usuário não permitido.")
    if not nfs:
        raise HTTPException(status_code=400, detail="NFS é obrigatório.")
    return await run_in_threadpool(listar_itens_sync, nfs)
